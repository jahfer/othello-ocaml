...
let rec append rope str = str |> of_string |> concat rope

let rec split_at rope pos = match rope with
  | Node(weight, lhs, rhs) when pos <= weight ->
    let (lhs', rhs') = split_at lhs pos
    in (lhs', concat rhs' rhs)
  | Node(weight, lhs, rhs) ->
    let (lhs', rhs') = split_at rhs (pos - weight)
    in (concat lhs lhs', rhs')
  | Leaf(str, length) ->
    let lhs = Str.string_before str pos
    and rhs = Str.string_after str pos
    in (of_string lhs, of_string rhs)

let rev_concat rhs lhs = concat lhs rhs

let insert str pos rope =
  let (lhs, rhs) = split_at rope pos
  in str |> append lhs |> rev_concat rhs
